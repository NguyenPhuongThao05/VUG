...
class Collector(object):
    ...
    def eval_batch_collect(
        self, scores_tensor: torch.Tensor, interaction, positive_u: torch.Tensor, positive_i: torch.Tensor, group=None
    ):
        """ Collect the evaluation resource from batched eval data and batched model output.
            Args:
                ...
                group (str, optional): The evaluation group, either 'overlap' or 'non_overlap'. Defaults to None.
        """
        suffix = f"_{group}" if group else ""

        if self.register.need(f'rec.items{suffix}'):
            _, topk_idx = torch.topk(scores_tensor, max(self.topk), dim=-1)  # n_users x k
            self.data_struct.update_tensor(f'rec.items{suffix}', topk_idx)

        # if self.register.need(f'rec.topk{suffix}'):
        if True:
            _, topk_idx = torch.topk(scores_tensor, max(self.topk), dim=-1)  # n_users x k
            ...
            self.data_struct.update_tensor(f'rec.topk{suffix}', result)

        if self.register.need(f'rec.meanrank{suffix}'):
            ...
            self.data_struct.update_tensor(f'rec.meanrank{suffix}', result)

        if self.register.need(f'rec.score{suffix}'):
            self.data_struct.update_tensor(f'rec.score{suffix}', scores_tensor)

        if self.register.need(f'data.label{suffix}'):
            self.label_field = self.config['LABEL_FIELD']
            self.data_struct.update_tensor(f'data.label{suffix}', interaction[self.label_field].to(self.device))
    ...
    def get_data_struct(self):
        ...
        # Reset outdated evaluation data
        for key in ['rec.topk', 'rec.meanrank', 'rec.score', 'rec.items', 'data.label']:
            for suffix in ["", "_overlap", "_non_overlap"]:
                full_key = key + suffix
                if full_key in self.data_struct:
                    del self.data_struct[full_key]
